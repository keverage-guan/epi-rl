...
parser.add_argument("--monitor_path", type=str,
                    default=f"/tmp/SEIR-RiskPPO-{datetime.datetime.now():%Y-%m-%d-%H-%M-%S-%f}/")
...
parser.add_argument("--gamma_risk", type=float, default=0.5,
                    help="Weight of the CVaR shortfall penalty; 0 recovers vanilla PPO.")
parser.add_argument("--alpha", type=float, default=0.2,
                    help="CVaR tail fraction used to estimate nu each rollout.")
parser.add_argument("--n_probe_episodes", type=int, default=8,
                    help="Rollouts used to estimate nu before each PPO rollout.")
...
def make_model() -> UK:
    ...
    mu = np.log(args.R0) * .6
    return UK(DELTA, args.R0, RHO, GAMMA, district_names, grouped_census, fl, mu, sde=True)

def wrap_env(env):
    env = NormalizedObservationWrapper(env)
    if args.outcome == "ar":
        env = NormalizedRewardWrapper(env)
    return RiskShapedReward(env, alpha=args.alpha)

def make_probe_env():
    env = SEIREnvironment(model=make_model(), n_weeks=N_WEEKS, step_granularity=GRANULARITY,
                          outcome=OUTCOME, model_seed=args.district_name,
...
risk_callback = RiskShapingCallback(env_factory=make_probe_env, gamma=args.gamma_risk,
                                    alpha=args.alpha, n_probe_episodes=args.n_probe_episodes)
progress_callback = ProgressLoggingCallback(total_timesteps=args.total_timesteps)
model.learn(total_timesteps=args.total_timesteps, progress_bar=False,
           callback=CallbackList([risk_callback, progress_callback]))
